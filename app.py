...
if submitted:
    folders: List[str] = [os.path.normpath(line.strip()) for line in folder_text.splitlines() if line.strip()]
    # Check which folders exist
    missing = [f for f in folders if not os.path.isdir(f)]
    if missing:
        st.warning(f"⚠️ These folders were not found:\n" + "\n".join(f"  - {f}" for f in missing))
    existing = [f for f in folders if os.path.isdir(f)]
    if not existing:
        st.error('❌ No valid folders found. Please check the paths above.')
        st.stop()
    with st.spinner(f'Loading sessions from {len(existing)} folder(s)...'):
        sessions = cached_load_sessions(existing)
    
    # Debug: show what was found
    if not sessions:
        st.error('No sessions found. Check your folder paths and contents.')
        # Try to find any CSV files for debugging using os.walk (more reliable than glob)
        found_csvs = []
        found_dirs = []
                ...
                for dirpath, dirnames, filenames in os.walk(folder):
                    for name in filenames:
                        if name.lower().endswith('.csv'):
                            found_csvs.append(os.path.join(dirpath, name))
        
        if found_dirs:
            st.info(f"✅ Searched {len(found_dirs)} folder(s):")
            for d in found_dirs:
                st.text(f"  - {d}")
                # Show directory tree structure (2 levels deep)
                try:
                    all_items = []
                    for root, dirs, files in os.walk(d):
                        ...
                        if level <= 2:  # Show up to 2 levels deep
                            indent = '    ' + '  ' * level
                            rel_path = os.path.relpath(root, d) if root != d else '.'
                            if rel_path == '.':
                                st.text(f"{indent}[{os.path.basename(root)}]")
                            else:
                                st.text(f"{indent}[{rel_path}]")
                            # Show CSV and ZIP files
                            csvs_in_dir = [f for f in files if f.lower().endswith('.csv')]
                            zips_in_dir = [f for f in files if f.lower().endswith('.zip')]
                            if csvs_in_dir:
                                st.text(f"{indent}  📄 CSV: {', '.join(csvs_in_dir[:3])}")
                                if len(csvs_in_dir) > 3:
                                    st.text(f"{indent}     ... and {len(csvs_in_dir) - 3} more")
                            if zips_in_dir:
                                st.text(f"{indent}  📦 ZIP: {', '.join(zips_in_dir[:3])}")
                except Exception as e:
                    st.text(f"    (Could not list contents: {e})")
        
        if found_csvs:
            st.success(f"Found {len(found_csvs)} CSV file(s):")
            for csv in found_csvs[:15]:  # Show first 15
                rel_path = os.path.relpath(csv, existing[0]) if existing else csv
                st.text(f"  - {rel_path}")
            if len(found_csvs) > 15:
                st.text(f"  ... and {len(found_csvs) - 15} more")
            st.warning("⚠️ CSV files found but none matched 'sensorlog' pattern. Looking for files with 'sensorlog' or 'sensor_log' in the name.")
        else:
            st.info("❌ No CSV files found in the specified folders. Check that the folder contains CSV files.")
        st.stop()
    
    # Filter out sessions without position data
    sessions_with_pos = [s for s in sessions if s.pos is not None and not s.pos.empty]
    if not sessions_with_pos:
        st.warning(f"Found {len(sessions)} session(s), but none have GPS position data (latitude/longitude).")
        st.info("Make sure your CSV files contain 'sensorlog_pos_*.csv' files with latitude and longitude columns.")
        st.stop()

    summary_rows: List[Dict] = []
    per_session_details = []
    for sess in sessions_with_pos:
